...
export class ResponseProcessor {
	...
	static setItemsPerPage(itemsPerPage: number): void {
		ResponseProcessor.items_per_page = itemsPerPage;
	...
	private static decodePageToken(token: string): number {
		...
		try {
			...
			const page = Number.parseInt(decoded.replace("page_", ""));
	...
	private static shouldPaginateArray(array: any[]): boolean {
		const should = array.length > ResponseProcessor.items_per_page;
		console.log("[Pagination] Should paginate array?", {
			arrayLength: array.length,
			itemsPerPage: ResponseProcessor.items_per_page,
	...
	private static shouldPaginateObject(obj: any): boolean {
		const should = Object.keys(obj).length > ResponseProcessor.items_per_page;
	...
	private static paginateObject(
		...
		const totalPages = Math.ceil(totalItems / ResponseProcessor.items_per_page);
		const currentPage = pageToken ? ResponseProcessor.decodePageToken(pageToken) : 1;

		console.log("[Pagination] Object pagination state:", {
			totalItems,
			totalPages,
			currentPage,
		});

		const startIndex = (currentPage - 1) * ResponseProcessor.items_per_page;
		const endIndex = startIndex + ResponseProcessor.items_per_page;
		...
		return {
			...
			metadata: {
				...
				hasNextPage,
				...(hasNextPage && {
					pageToken: ResponseProcessor.generateNextPageToken(currentPage + 1),
				}),
			},
		};
	}
	...
	private static paginateArray<T>(
		...
		console.log("[Pagination] Array pagination state:", {
			totalItems,
			totalPages,
			currentPage,
		});
	...
	static processResponse(response: any, pageToken?: string): any {
		...
		if (Array.isArray(response)) {
			...
			if (ResponseProcessor.shouldPaginateArray(arrayResponse)) {
				const { items, metadata } = ResponseProcessor.paginateArray(
					arrayResponse,
					pageToken,
				);
		...
		if (typeof response === "object" && response !== null) {
			...
			for (const key in processed) {
				...
				if (
					...
					!ResponseProcessor.shouldSkipPagination(processed, key)
				) {
					if (ResponseProcessor.shouldPaginateArray(processed[key])) {
						const result = ResponseProcessor.paginateArray(processed[key], pageToken);
					...
					if (ResponseProcessor.shouldPaginateObject(processed[key])) {
						const result = ResponseProcessor.paginateObject(processed[key], pageToken);
						processed[key] = result.items;
						if (result.metadata) {
							paginatedField = key;
							paginationMetadata = result.metadata;
					...
					const nestedResult = ResponseProcessor.processResponse(
						processed[key],
						pageToken,
					);
