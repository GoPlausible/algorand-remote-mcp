 ...
 */
export async function registerWalletTools(
	server: McpServer,
	env: Env,
	props: Props,
): Promise<void> {
	...
	server.tool(
		...
		async () => {
			try {
				...
				if (!roleId || !entityId) {
					console.log(
						`provider: ${props.provider}, email: ${props.email}, clientId: ${props.clientId}, userId: ${props.id}`,
					);
				...
				if (publicKeyResult.success && !publicKeyResult.error) {
					...
					console.log(
						`Getting public key for ${props.email} with provider ${props.provider}`,
					);
					...
					if (!newPublicKeyResult.success || !newPublicKeyResult.publicKey) {
						throw new Error(
							newPublicKeyResult.error || "Failed to get public key from vault",
						);
					}
	...
	// Get wallet assets
	server.tool("wallet_get_assets", "Get the assets for the configured wallet", {}, async () => {
		if (!env.ALGORAND_ALGOD) {
			return {
				content: [
					{
		...
		async () => {
			try {
				...
				if (props?.accessToken && props?.provider) {
					...
					console.log(
						`Including token and provider in logout request: provider=${props.provider}, token length=${props.accessToken.length}`,
					);
				} else {
				...
				if (!response.ok) {
					...
					console.error(
						`Logout failed with status: ${response.status}, response: ${errorText}`,
					);
				...
				return ResponseProcessor.processResponse({
					success: true,
					message:
						"Successfully logged out. You will need to re-authenticate on your next request.",
					user: props.email,
					provider: props.provider,
