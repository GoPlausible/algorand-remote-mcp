...
import { getPublicKey, getUserAddress, signWithTransit } from "../../../utils/vaultManager";
	...
	server.tool(
		...
		"Get an optimized swap quote from Haystack Router — a DEX aggregator that finds the best swap route across multiple Algorand DEXes (Tinyman V2, Pact, Folks) and LST protocols (tALGO, xALGO). Returns the best-price quote with route details, USD values, and price impact. All amounts are in base units (e.g., 1000000 = 1 ALGO).",
		{
			fromASAID: z
				.number()
				.int()
				.describe("Input asset ID (0 = ALGO, 31566704 = USDC, 312769 = USDt, etc.)"),
			toASAID: z
				.number()
				.int()
				.describe("Output asset ID (0 = ALGO, 31566704 = USDC, 312769 = USDt, etc.)"),
			amount: z
				.number()
				.int()
				.describe("Amount in base units (e.g., 1000000 = 1 ALGO with 6 decimals)"),
			type: z
				.enum(["fixed-input", "fixed-output"])
				.default("fixed-input")
				.describe("Quote type: fixed-input or fixed-output"),
			address: z
				.string()
				.optional()
				.describe("User Algorand address (optional, needed for auto opt-in detection)"),
			maxGroupSize: z
				.number()
				.int()
				.optional()
				.default(16)
				.describe("Maximum transactions in atomic group"),
	...
	server.tool(
		...
		"Execute an optimized token swap via Haystack Router — gets the best route across multiple DEXes (Tinyman V2, Pact, Folks) and LST protocols, then signs with the vault wallet and submits. This is an all-in-one tool: quote → sign → submit → confirm. All amounts are in base units.",
		{
			...
			slippage: z
				...
				.optional()
				.default(1)
				.describe("Slippage tolerance percentage (e.g., 1 = 1%)"),
			type: z
				.enum(["fixed-input", "fixed-output"])
				.default("fixed-input")
				.describe("Quote type"),
			note: z
				.string()
				.optional()
				.describe("Optional note to attach to the input transaction"),
			maxGroupSize: z
				.number()
				.int()
				.optional()
				...
				const signer = async (
					...
					for (const index of indexesToSign) {
						...
						if (!signatureResult.success || !signatureResult.signature) {
							throw new Error(
								`Failed to sign transaction at index ${index} with vault`,
							);
						}

						const signature = new Uint8Array(
							Buffer.from(signatureResult.signature, "base64"),
						);
