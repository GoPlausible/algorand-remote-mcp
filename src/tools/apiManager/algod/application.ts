		...
		async ({ appId, name }) => {
			...
			try {
				...
				const response = await algodClient
					.getApplicationBoxByName(Number(appId), nameBytes)
					.do();
		...
		async ({ appId, maxResults }) => {
			...
			try {
				...
				const response = await algodClient
					...
					.max(Number(maxResults))
					.do();

				// Format the response to include readable box names
				const formattedResponse = {
					boxes: response.boxes
						? response.boxes.map((box: any) => {
								try {
									// Use a safe way to decode box name
									const nameAsString = new TextDecoder().decode(
		...
		async ({ appId }) => {
			...
			try {
				...
				const processedState = globalState.map((stateItem: any) => {
					...
					let processedValue: Record<string, unknown> | undefined;
					if (value.type === 1) {
						// bytes
