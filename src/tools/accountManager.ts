		...
		async ({ mnemonic }) => {
			try {
				...
				return ResponseProcessor.processResponse({
					...
					message:
						"This shows the address and sk associated with the mnemonic. For security reasons, the private key is not stored anywhere and solely for user external use purposes.",
				});
			} catch (error: any) {
				return {
					content: [
		...
		async ({ address }) => {
			try {
				...
				return ResponseProcessor.processResponse({
					...
					message:
						"This shows the public key associated with the given Algorand address.",
				});
			} catch (error: any) {
				return {
					content: [
