 ...
 */
export async function registerGeneralTransactionTools(
	server: McpServer,
	env: Env,
	props: Props,
): Promise<void> {
		...
		async ({ sender, receiver, amount, note, closeRemainderTo, rekeyTo, fee }) => {
			...
			try {
				...
				const txn = algosdk.makePaymentTxnWithSuggestedParamsFromObject({
					...
					closeRemainderTo,
					rekeyTo,
					suggestedParams: params.fee
						? { ...params, fee: Number(params.fee), flatFee: true }
						: params,
				});
				...
				return ResponseProcessor.processResponse({
					txID: txn.txID(),
					encodedTxn: Buffer.from(algosdk.encodeUnsignedTransaction(txn)).toString(
						"base64",
					),
					txnInfo: {
						sender: sender,
	...
	server.tool(
		...
		async ({ encodedTxn }) => {
			try {
				...
				console.log(
					`Signing transaction for ${props.email} with provider ${props.provider}`,
				);
				...
				console.log(
					...
					new Uint8Array(Buffer.from(encodedTxn, "base64")),
				);
	...
	server.tool(
		...
		async ({ encodedTxn, mnemonic }) => {
			try {
				...
				// Get the signature from the signed blob
				const signature = Buffer.from(signatureResult.blob);
				console.log("Signature:", signature);
		...
		async ({ signedTxn }) => {
			...
			try {
				...
				const response = await algodClient
					.sendRawTransaction(new Uint8Array(decodedTxn))
					.do();
	...
	server.tool(
		...
		"Create a key registration transaction on Algorand",
		{
			...
			voteKey: z
				.string()
				.describe("The root participation public key (58 bytes base64 encoded)"),
			...
			nonParticipation: z
				.boolean()
				.optional()
				.describe("Mark account as nonparticipating for rewards"),
		...
		async ({
			...
			try {
				...
				// Create key registration transaction
				let txn: algosdk.Transaction | undefined;
