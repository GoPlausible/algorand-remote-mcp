	...
	server.tool(
		...
		"Create a new smart contract application on Algorand",
		{
			...
			extraPages: z
				...
				.max(3)
				.default(0)
				.describe("Extra pages allocated to the app"),
		},
		async ({
			creator,
			approvalProgram,
	...
	server.tool(
		...
		"Update an existing smart contract application on Algorand",
		{
			...
			accounts: z
				.array(z.string())
				.optional()
				.describe("Accounts to be passed to the application"),
			foreignApps: z
				.array(z.number())
				.optional()
				.describe("Foreign apps to be passed to the application"),
			foreignAssets: z
				.array(z.number())
				.optional()
				.describe("Foreign assets to be passed to the application"),
			note: z.string().optional().describe("Optional transaction note"),
		},
		async ({
			sender,
		...
		async ({
			...
			try {
				...
				switch (onComplete) {
					...
					case "update":
						throw new Error(
							"Application update requires approval and clear programs. Use update_application instead.",
						);
					case "delete":
