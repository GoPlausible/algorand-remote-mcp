...
import { getPublicKey, /* , ensureUserAccount */ signWithTransit } from "../../utils/vaultManager";
		...
		case "appl": // Application call transaction
			return algosdk.makeApplicationCallTxnFromObject({
				...
				approvalProgram: params.approvalProgram
					? new Uint8Array(Buffer.from(params.approvalProgram, "base64"))
					: undefined,
				clearProgram: params.clearProgram
					? new Uint8Array(Buffer.from(params.clearProgram, "base64"))
					: undefined,
...
export function registerGroupTransactionTools(server: McpServer, env: Env, props: Props): void {
	...
	server.tool(
		...
		"To group transactions in atomic way (one fails all fail), assign a group ID to a set of transactions for atomic execution",
		{
			encodedTxns: z
				.array(z.string())
				.describe("Array of base64-encoded unsigned transactions"),
		},
		async ({ encodedTxns }) => {
			try {
				// Decode transactions
	...
	server.tool(
		...
		"Sign an atomic transaction group. This is the default way of signing transaction groups by agents using this MCP. This uses the vault account binding to user OAuth account used to authenticate user to this MCP.",
		{
			...
			keyName: z
				.string()
				.describe("Key name of the signer for all transactions in the group"),
		},
		async ({ encodedTxns, keyName }) => {
			try {
				...
				console.log(
					`Signing atomic transaction group for ${props.email} with provider ${props.provider}`,
				);
				...
				if (publicKeyResult.success && !publicKeyResult.error) {
					...
					const signaturePromises = groupedEncodedTxns.map(async (txn) => {
						...
						console.log(
							"Encoded transaction buffer signing:",
						...
						const txnObj = msgpack.decode(
							algosdk.encodeUnsignedTransaction(decodedTxn),
						);
				...
				if (failedSignatures.length > 0) {
					throw new Error(
						`Failed to sign ${failedSignatures.length} transaction(s) in the group`,
					);
				}
	...
	server.tool(
		...
		async ({ signedTxns }) => {
			...
			try {
				...
				const decodedTxns = signedTxns.map(
