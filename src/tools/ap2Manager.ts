...
async function generateVerifiableCredential(
	...
	if (mandateType === "payment_mandate" || mandateType === "intent_mandate") {
		...
		signatureResult = await signWithTransit(
			env,
			finalEncodedMandateBase64,
			props.email,
			props.provider,
		);
		...
		"Create an AP2 intent, cart or payment mandate for AP2 process and flow using fields: id, type (mandate type), items, total, currency, merchant_public_key, payment_requirements, merchant_agent (id) and cart_request_id then returns an stringified object containing verifiableCredential and verifiableCredentialLink.",
		{
			mandate: z.object({
				...
				type: z
					...
					.describe("Mandate type for AP2 flow"),
				...
				total: z
					.string()
					.describe(
						"Total amount in smallest unit given currency decimals (e.g., 1000000 = 1 USDC)",
					),
				items: z
					.array(
						z.object({
				...
				merchant_agent: z
					.string()
					.optional()
					.describe("DID or identifier of merchant agent"),
				refund_period: z
					.string()
					.optional()
					.describe("Refund period in ISO-8601 duration format (e.g., P30D = 30 days)"),
				cart_request_id: z
					.string()
					.optional()
					.describe("Associated cart or payment request ID"),
			}),
		},
		async ({ mandate }) => {
			...
			try {
				...
				const shopperAddress = algosdk.encodeAddress(
					Buffer.from(publicKeyResult.publicKey, "base64"),
				);
				...
				if (
					...
					mandateData.merchant_public_key.length === 58
				) {
					mandateData.merchant_public_key = Buffer.from(
						algosdk.decodeAddress(mandateData.merchant_public_key).publicKey,
					).toString("base64");
					console.log(
						"Converted merchant public key to base64:",
						mandateData.merchant_public_key,
					);
				}
											...
											method_data: [
												{
													...
													data: {
														payment_requirements:
															mandateData.payment_requirements,
													},
												},
											],
											details: {
