...
export function registerUtilityTools(server: McpServer, env: Env, props: Props): void {
	...
	server.tool(
		...
		"Encode a public key to an Algorand address",
		{
			publicKey: z
				.string()
				.describe("Public key in hexadecimal format to encode into an address"),
		},
		async ({ publicKey }) => {
			try {
	...
	server.tool(
		...
		"Decode an Algorand address to a public key",
		{
			address: z
				.string()
				.describe("Address in standard Algorand format (58 characters) to decode"),
		},
		async ({ address }) => {
			try {
	...
	server.tool(
		"algorand_mcp_skill",
		"Access comprehensive skill definition for using Algorand Remote MCP, including capabilities, workflows, best practices, and tool reference.",
		{},
		async () => {
			try {
