...
import { type SupportedNetwork, Swap, SwapType, poolUtils } from "@tinymanorg/tinyman-js-sdk";
 ...
 */
export async function registerTinymanTools(
	server: McpServer,
	env: Env,
	props: Props,
): Promise<void> {
	...
	server.tool(
		...
		"Execute a swap with a fixed input amount",
		{
			...
			slippage: z
				...
				.optional()
				.default(0.05)
				.describe("Slippage tolerance (e.g., 0.05 for 5%)"),
			network: z
				.enum(["mainnet", "testnet"])
				.optional()
				.default("mainnet")
				.describe("Algorand network"),
		},
		async (args) => {
			try {
				const {
				...
				console.log(
					"[TINYMAN_SWAP] Asset Decimals - In:",
					assetInDecimals,
					"Out:",
					assetOutDecimals,
				);
				...
				console.log("[TINYMAN_SWAP] Quote structure check:", {
					...
					quoteDataKeys: fixedInputSwapQuote.data
						? Object.keys(fixedInputSwapQuote.data)
						: [],
				...
				for (const txnData of quoteData.transactions) {
					if (txnData.type === "appl") {
						...
						const appArgs = txnData.args
							? txnData.args.map(
									(arg: string) => new Uint8Array(Buffer.from(arg, "base64")),
								)
							: [];
				...
				for (const txn of orderedTransactions) {
					...
					const finalEncodedTxnTagged = new Uint8Array(
						TAG.length + finalEncodedTxn.length,
					);
					finalEncodedTxnTagged.set(TAG);
					finalEncodedTxnTagged.set(finalEncodedTxn, TAG.length);

					const finalEncodedTxnBase64 =
						Buffer.from(finalEncodedTxnTagged).toString("base64");

					// Sign with Vault
					if (!enhancedEnv.email || !enhancedEnv.provider) {
						throw new Error(
							"Email and provider must be provided for transaction signing",
						);
					}
					...
					const encodedSignedTxn = new Uint8Array(
		...
		async (args) => {
			try {
				...
				console.log("[TINYMAN_SWAP] Quote structure check:", {
					...
					quoteDataKeys: fixedOutputSwapQuote.data
						? Object.keys(fixedOutputSwapQuote.data)
						: [],
					// Use type assertion to access properties safely
