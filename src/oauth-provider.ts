...
class OAuthProviderImpl {
	...
	async fetch(request: Request, env: any, ctx: ExecutionContext): Promise<Response> {
		...
		if (!env.OAUTH_PROVIDER) {
			...
			console.log(
				"[OAUTH_PROVIDER] OAuth helpers injected: env.OAUTH_PROVIDER using createOAuthHelpers",
			);
		}
	...
	private async handleAuthorizationCodeGrant(
		...
		grantData.authCodeId = undefined;
		grantData.codeChallenge = undefined;
		grantData.codeChallengeMethod = undefined;
		grantData.authCodeWrappedKey = undefined;
	...
	private async handleApiRequest(
		...
		if (!apiHandler) {
			...
			return this.createErrorResponse(
				"invalid_request",
				"No handler found for API route",
				404,
			);
		}
...
class OAuthHelpersImpl implements OAuthHelpers {
	...
	async completeAuthorization(
		...
		if (options.request.responseType === "token") {
			console.log(
				"[OAUTH_PROVIDER] Implicit flow requested, generating access token directly",
			);
		...
		console.log(
			"[OAUTH_PROVIDER] Authorization code flow requested, generating authorization code",
		);
		...
		if (options.request.state) {
			...
			console.log(
				"[OAUTH_PROVIDER] Adding client ID to redirect URL:",
